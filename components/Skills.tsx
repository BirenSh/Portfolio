    ...
    <section id="skills" className="py-20 border-t border-slate-200 dark:border-slate-800">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {SKILLS.map((group) => (
          <div 
            key={group.category}
            className="p-6 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 hover:border-emerald-500 dark:hover:border-emerald-500/50 transition-all duration-300 hover:shadow-xl hover:shadow-emerald-500/5"
          >
            ...
            <div className="space-y-4">
              {group.skills.map((skill) => (
                <div key={skill.name} className="flex items-center gap-3 group/item">
                  <div className="p-2 bg-slate-50 dark:bg-slate-800 rounded-lg group-hover/item:bg-emerald-50 dark:group-hover/item:bg-emerald-950/30 transition-colors">
                    {skill.icon}
                  </div>
